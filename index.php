<!-- ************************************************************************************************************** -->
<!-- *************************************** Created By Marika Lee 8/7/2017 *************************************** -->
<!-- ************************************************************************************************************** -->

<!DOCTYPE html>
<html lang="en">
   <head>
      <title>Home | Menlo Weight Loss Clinic</title>
      <link rel="shortcut icon" href="icon.png" />
      <meta property="og:image" content="images/manny.jpg"/>   
      <meta charset="utf-8" />      
      <meta property="og:locale" content="en_US" />
      <meta property="og:type" content="website" />
      <meta property="og:title" homecontent="Menlo Weight Loss Clinic" />
      <meta property="og:description" content="Menlo Weight Loss Clinic" />
   </head>   
   <header>
      <link rel='stylesheet' id='googlefonts-css'  href='http://fonts.googleapis.com/css?family=Josefin+Sans:400|Josefin+Sans:400|Josefin+Sans:600italic|Josefin+Sans:600|Josefin+Sans:700|Josefin+Sans:400italic&subset=latin,latin-ext' type='text/css' media='all' />
      <link rel="stylesheet" href="css/style.css" type="text/css" media="screen" title="no title" charset="utf-8"/>
      <link rel="stylesheet" href="css/mobile.css" type="text/css" media="screen" title="no title" charset="utf-8"/>
      <link rel="stylesheet" href="css/slicknav.css" />
      <link href='https://fonts.googleapis.com/css?family=Dancing%20Script' rel='stylesheet'>
      <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
      <link rel = "stylesheet" type = "text/css" href = "style.css" />
      <script src="http://code.jquery.com/jquery-1.10.2.min.js"></script>
      <script src="js/jquery.infinitescroll.js"></script>
      <script src="js/jquery.slicknav.js"></script>
      <script src="js/retina-1.1.0.min.js"></script>
      <script src="js/jquery.carouFredSel-6.2.1.js"></script>
      <script src="js/scripts.js"></script> 
      <meta name="viewport" content="width=device-width, initial-scale=1">
      <script>
         var siteWidth = 1280;
         var scale = screen.width /siteWidth
         document.querySelector('meta[name="viewport"]').setAttribute('content', 'width='+siteWidth+', initial-scale='+scale+'');
      </script>
   </header>
   <body class="home" page page-id-147 page-template-default">
      <header id="header" >
         <div class="header_bottom">
            <div class="container" style="margin:0px; margin-left: 30px ">
               <div class="header_menu">
                <ul id="main_header_menu" class="menu">
                     <li id="menu-item-232" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-232"><a href="http://www.menloweightloss.com">HOME</a></li>
                     <li id="menu-item-232" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-232"><a href="http://www.menloweightloss.com/about">ABOUT</a></li>
                     <li id="menu-item-232" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-232"><a href="http://www.menloweightloss.com/clinic">CLINIC</a></li>
                     <li id="menu-item-533" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children menu-item-533">
                        <a>BLOG</a>
                        <ul class="sub-menu">
                        <li id="menu-item-320" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-320"><a href="http://www.menloweightloss.com/metrics-for-metabolic-fitness">Metrics For Metabolic Fitness</a></li>
                        </ul>
                     </li>
                     
                     <li id="menu-item-232" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-232"><a href="http://www.menloweightloss.com/contact">CONTACT</a></li>
                    
                  </ul>
               </div>
               <!--//header_menu--> 
               <div class="right" style="margin:20px">
                  <div class="header_social">
                     <a href="https://www.schedulicity.com/Scheduling/Default.aspx?business=TMH5WJ#/servicestarget=_blank"target="_blank"><img src="images/schedule.png" alt="schedule" style="height:25px;"/></a>
                     <a href="https://twitter.com/drmannylam"target="_blank"><img src="images/twitter-icon.png" alt="twitter" /></a>
                     
                     <a href="https://www.linkedin.com/in/manuel-lam-11478811"target="_blank"><img height="25px" src="images/linkedin.png" /></a>
                     <div class="clear"></div>
                  </div>
                  <!--//header_social-->        
                  <div class="clear"></div>
               </div>
               <!--//right-->
               <div class="clear"></div>
            </div>
            <!--//container-->
         </div>
         <!--//header_bottom-->  
      </header>
      <!--//header-->
      <div class="header_spacing"></div>
      <div class="container">
         <div class="logo_cont">
              <div class = "title">Menlo Weight Loss</div>    
             <div class = "italic">"Let food be thy medicine and medicine be thy food." -Hippocrates</div>       
         </div>
         <!--//logo_cont-->
         <div class="clear"></div><br>
      </div>
      <!--//container-->   
      <div id="content">
         <div class="container">
            <div id="single_cont">
               <div class="single_left">
                  <div class="single_inside_content">

                        <div class="summary">
                        Dr. Manuel Lam investigates for and treats a wide range of weight-related <strong>metabolic disorders</strong>. This includes:
                        <ul>
                           <li><strong>Overweight</strong>,</li>
                           <li><strong>Obesity</strong>,</li>
                           <li><strong>Prediabetes</strong>, </li>
                           <li><strong>Diabetes</strong>, </li>
                           <li><strong>High blood pressure</strong>, </li>
                           <li><strong>High triglycerides</strong>, </li> 
                           <li><strong>Polycystic Ovarian Syndrome</strong>, and </li>
                           <li><strong>Fatty liver</strong>.</li>
                        </ul>
                        </div>

                        <div class="summary">
                       We understand that <strong>weight gain</strong> and <strong>metabolic syndrome</strong> goes far beyond the oversimplified paradigm of simply <i>eating too much</i> and <i>moving too little</i>.  It is complicated, and we seek to shed light on the unique challenges that each individual faces, including <strong>hormonal imbalances</strong>, <strong>genetics</strong>, <strong>gut microbiome</strong>, <strong>medications</strong>, <strong>stress</strong>, <strong>sleep</strong>, <strong>cultural</strong> and <strong>social influences</strong>. As a result, there is no one-size-fits-all intervention that will work for all people to achieve successful long-term weight loss.
                        </div>

                        <div class="summary">
                        Our program considers the role that <strong>hormones</strong> (insulin, in particular) play in stimulating fat accumulation and weight gain. Therefore, our philosophy centers around <strong>nutritional ("food as medicine")</strong>, <strong>lifestyle</strong>, and <strong>medical</strong> strategies that aim to <strong>lower insulin</strong>, <strong>reverse insulin resistance</strong>, and <strong>lose body fat</strong>. These strategies include:
                        <ul>
                           <li> <strong>Personalized intermittent fasting protocols</strong></li>

                           <li>Eating <strong>whole, natural, nutrient dense non-processed, foods</strong></li>

                           <li>Increasing <strong>natural</strong>, <strong>healthy fats</strong></li>
                           
                           <li>Avoiding <strong>added sugars</strong>, <strong>starches and refined grains</strong></li>
                        </ul>

                        <div class="summary">Dr. Lam provides <strong>contextual</strong> medical advice regarding pre-existing medical conditions, medications, and metabolic laboratory testing. Under his medical supervision, patients <strong>reduce their weight and body fat percentage</strong>, <strong>lower their blood pressure</strong> and <strong>reverse their type 2 diabetes</strong>, while <strong>getting them <i>off</i> their prescription medications</strong>.</div>

                        </div>                     
                        </div>

                     <!-- <center><div class="scheduler" style="margin-top:50px">
                       <script type="text/javascript">
                          new function() {
                              var html = '<style>.schedulicity-embed-mobile { display: none; } .schedulicity-embed { position: relative; border: black 2px solid; background-color: white; width: 100%; max-width: 640px; } .schedulicity-embed > iframe { border: none; width: 100%; padding: 0; margin: 0; height: 480px; z-home: 1; } @media only screen and (max-width: 480px) { .schedulicity-embed { display: none; } .schedulicity-embed-mobile { display: block; } }</style>';
                              html += '<div class="schedulicity-embed">';
                              html += '<iframe src="https://www.schedulicity.com/scheduling/TMH5WJ?embed" allowtransparency="true" frameborder="0"></iframe>';
                              html += '</div>';
                              html += '<div class="schedulicity-embed-mobile">';
                              html += '<a href="https://www.schedulicity.com/scheduling/TMH5WJ" target="_blank"><button style="cursor: pointer; background-color: #14aec9; font-size: 14pt; color: white; padding: 3px 6px; text-align: center; font-family: sans-serif; border: 1px solid #14aec9;">schedule now</button></a>';
                              html += '</div>';

                              document.write(html);
                          }();
                       </script><br>
                      </div></center> -->

                  <!--//single_inside_content-->
                  <br /><br />
               </div>
               <!--//single_left-->
               <div id="sidebar">
                  <div class ="side_box">
                    <img src="images/manny.jpg" style='width:100%;' border="0" alt="Null">        
                  </div>

                  <div class="side_box">
                  <p style="margin-bottom:5px"><strong>HOURS:</strong> TUE, WED, FRI 9AM-5PM</p>
                  <p>845 Oak Grove Suite 110 Menlo Park, CA</p>
               </div>
                  <div class="side_box">
                     <a class="button" href="https://www.schedulicity.com/Scheduling/Default.aspx?business=TMH5WJ#/services">Schedule Complimentary Consultation</a>
                     <p><a href="https://obesitymedicine.org/"><img src="images/oma.png" alt="oma" width="200px"/></a></p>
                  </div>
                  
                  <div class="side_box">
                     <div class="twitter">
                        <a class="twitter-timeline" data-height="500" href="https://twitter.com/drmannylam">Tweets by drmannylam</a> <script async src="//platform.twitter.com/widthdgets.js" charset="utf-8"></script><br><br>
                     </div>

                     <iframe src="https://snapwidget.com/embed/414737" class="snapwidget-widget" allowTransparency="true" frameborder="0" scrolling="no" style="border:none; overflow:hidden; width:350px; height:390px"></iframe>
                  </div>
                  
                  
               </div>
               <!--//sidebar-->        
               <div class="clear"></div>
            </div>
            <!--//single_cont-->
         </div>
         <!--//container-->
      </div>
      <!--//content-->
      <div class="footer_copyright_cont">
         <div class="footer_copyright">
          <div class="container">
               © 2017 The Metabolic Health Clinic, Inc Rights Reserved  
               <div class="clear"></div>
            </div>
            <!--//container-->
         </div>
         <!--//footer_copyright-->
      </div>
      <!--//footer_copyright_cont-->
      <script type="text/javascript"><!--
         wpa2a.targets=[
         {title:'RESUME',url:'http://themetabolichealthclinic.com/'}];
         wpa2a_done=true;if(wpa2a.script_ready&&!wpa2a.done)wpa2a.init();wpa2a.script_load();
         //-->
      </script>
      <div style="display:none">
      </div>
      <script type="text/javascript">
         !function(d,s,id){
            var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';
            if(!d.getElementById(id)){
               js=d.createElement(s);
               js.id=id;js.src=p+"://platform.twitter.com/widgets.js";
               fjs.parentNode.insertBefore(js,fjs);
            }
         }(document,"script","twitter-wjs");
      </script>
     
      
    
      <script type='text/javascript' src='http://s0.wp.com/wp-content/js/devicepx-jetpack.js?ver=201720'></script>
  
   
     
      <script type='text/javascript' src='http://stats.wp.com/e-201720.js' async defer></script>
      <script type='text/javascript'>
         _stq = window._stq || [];
         _stq.push([ 'view', {v:'ext',j:'1:3.6',blog:'77606535',post:'147',tz:'0',srv:'menloweightloss.com'} ]);
         _stq.push([ 'clickTrackerInit', '77606535', '147' ]);
      </script>
   </body>
</html>

